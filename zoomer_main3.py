...
from utils import new_collate_fn
from zoomer_3 import Zoomer
from dataloader3 import GRDataset
...
def main():
    ...
    train_loader = DataLoader(train_data, batch_size = 512, shuffle = True, collate_fn = new_collate_fn)
    valid_loader = DataLoader(valid_data, batch_size = 512, shuffle = True, collate_fn = new_collate_fn)
...
def trainForEpoch(train_loader, model, optimizer, epoch, num_epochs, criterion, log_aggr=1):
    ...
    for i, (uids, mids, qids, labels, nb, nb2) in enumerate(train_loader):
        uids = uids.to(device)
        mids = mids.to(device)
        qids = qids.to(device)
        ...
        optimizer.zero_grad()
        outputs = model(uids, mids, qids, nb, nb2)
...
def validate(valid_loader, model):
    ...
    with torch.no_grad():
        for uids, mids, qids, labels, nb, nb2 in tqdm(valid_loader):
            uids = uids.to(device)
            mids = mids.to(device)
            qids = qids.to(device)
            labels = labels.to(device)

            preds = model(uids, mids, qids, nb, nb2)
